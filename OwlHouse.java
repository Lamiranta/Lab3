...
public class OwlHouse extends Location
{
     ...
     * If there's no such guest then throws an exception.
     *
     * @param num specific number of guest.
     * @return    name of a guest or an exception.
     */
    public String getGuest(int num) throws GuestNumberException
    {
        try
        {
            if (num < 0 || num > guest.length)
                throw new GuestNumberException("No actual guest with this number");
            return this.guest[num];
        }
        catch (GuestNumberException e)
        {
